...
from app.subscription.service import build_subscription
...
@router.get("/sub-v2/{uuid}")
async def sub_v2(uuid: str):

    try:
        result = await build_subscription(uuid)
        ...
        print(f"Ошибка генерации подписки: {e}")

        return Response(
            "Failed to generate subscription",
            status_code=500,
